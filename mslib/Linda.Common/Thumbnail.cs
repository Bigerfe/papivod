...
public class Thumbnail
{
    ...
    /// <summary>
    /// 生成带文字的图片 并返回此图片的文件全名
    /// </summary>
    /// <param name="strword"></param>
    /// <returns></returns>
    public static string BuildImage(string strword,string savepath)
    {
        Bitmap image = null;
        Graphics graphics = null;
      
        int width =strword.Length*10;
     ...
     //   string[] textArray = strword .Split(new char[] { ' ' });
        for (int i = 0; i <strword.Length; i++)
        {
            ...
            graphics.DrawString(strword[i].ToString(), font, brush, (float)(i * 9), 1f);
        }

        string filename = DoClass.GetRandNum()+".jpg";
        image.Save(Utils.GetMapPath(savepath)+filename, ImageFormat.Jpeg); 
        graphics.Dispose();
        image.Dispose();
