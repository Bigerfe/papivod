...
public class Utils
{
    ...
    /// <returns></returns>
    public static string ShitEncode(string s, string strs)
    {
        string bw = strs;
        if (bw == null || 0 == bw.Length)
        {
    ...
    {
        return float.Parse(StrToFloat(value, 0).ToString()).ToString("#.00");
    }
    ...
    /// </summary>
    /// <returns></returns>
    public static string GetCurWeekOneDate()
    {
        string date = DateTime.Now.AddDays(0 - Convert.ToDouble(DateTime.Now.DayOfWeek)).ToString();
    ...
    {
        string date = DateTime.Parse(GetCurWeekOneDate()).AddDays(7).ToString();
        return date;
    }
    ...
    /// </summary>
    /// <returns></returns>
    public static string GetCurMonthOneDay()
    {
        string date = DateTime.Now.AddDays(0 - Convert.ToDouble(DateTime.Now.Day)).ToString();
        return date;
    }
    ...
    /// </summary>
    /// <returns></returns>
    public static string GetCurMonthEndDay()
    {
        string date = DateTime.Parse(GetCurMonthOneDay()).AddMonths(1).ToString();
        return date;
    }
    ...
    public static string ChkSQL(string str)
    {
        ...
        {
            str = str.Replace("'", "''").Replace("<", "").Replace(">", "").Replace("<script>", "").Replace("</script>", "").Replace("src=", "");
            str2 = str;
        }
        return str2;
    }
   ...
   /// <returns></returns>
    ...
    public static string Get6RandNum()
    {
        ...
        string date = GetDateTimeF().Replace(":", "").Replace(" ", "").Replace("-", "");

        return date.Substring(date.Length - 6);
    }
    ...
    /// 返回时间段的sql查询语句
    /// </summary>
    /// <param name="columname">字段名称</param>
    /// <param name="stime">起始时间</param>
    /// <param name="etime">结束时间</param>
    /// <returns>返回sql语句</returns>
    public static string GetSqlWithTime(string columname, string stime, string etime)
    {

        stime = Utils.GetDate(stime, "");
        etime = Utils.GetDate(etime, "");

        string sql = "";
        if (stime == "" && etime == "")
            sql = "";
        else if (stime != "" && etime != "")
            sql = columname + " between '" + stime + "' and '" + etime + "'";
        else if (stime != "" && etime == "")
            sql = columname + " >= '" + stime + "'";
        else if (stime == "" && etime != "")
            sql = columname + " <= '" + etime + "'";

        return sql != "" ? "and " + sql : sql;

    }
    /// <summary>
    /// 返回一个虚拟路径的最终的文件名称 路径如  /webfile/priimg/aa.gif
    /// </summary>
    /// <param name="path">路径</param>
    /// <returns>aa.gif</returns>
    public static string GetVituralpathFileName(string path)
    {
    ...
    /// 给当前的下拉框 插入数值的项 
    /// </summary>
    /// <param name="maxcount">最大值</param>
    /// <param name="bdefaultselect">是否默认的选择项</param>
    /// <param name="text"></param>
    /// <param name="text"></param>
    public static void InsertItemToDropDownList(DropDownList drop, int maxcount, bool bdefaultselect, string text, string value)
    {
        for (int i = 1; i <= maxcount; i++)
        {
            drop.Items.Add(new ListItem(i.ToString(), i.ToString()));
        }

        if (bdefaultselect)
            drop.Items.Insert(0, new ListItem(text, value));

        if (maxcount <= 0)
            drop.Items.Insert(0, new ListItem("没有库存", ""));
    }


    /// <summary>
    /// 返回此cheboxlist控件的值的列表 
    /// </summary>
    /// <param name="chelist">控件</param>
    /// <param name="splitstr">多个值之间的分割字符 默认用 ,</param>
    /// <returns></returns>
    public static string GetCheckBoxListValueList(CheckBoxList list, string splitstr)
    {
    ...
    /// <summary>
    /// 设置已经绑定数据的cheboxlist 的选中项
    /// </summary>
    /// <param name="list">控件</param>
    /// <param name="strvaluelist">值的列表 如 1,2,3</param>
    /// <param name="spliter">值之间的分割字符</param>
    public static void SetCheckBoxListSelected(CheckBoxList list, string strvaluelist, string spliter)
    {
        foreach (ListItem item in list.Items)
        {
            if ((strvaluelist + spliter).IndexOf(item.Value + spliter) != -1)
            {
                item.Selected = true;
            }
        }
    ...
    /// <returns></returns>
    public static bool IsInStr(string oldstr, string[] arr)
    {
        foreach (string s in arr)
        {
            if (oldstr.IndexOf(s) != -1)
                return true;
        }
        return false;
    }
